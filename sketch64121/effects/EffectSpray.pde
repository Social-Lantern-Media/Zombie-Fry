class EffectSpray extends Effect {
  ...
  EffectSpray(int width, int height) {super(width,height);}
  ...
  PImage end(boolean ignored) { return end(); }
  @Override
  void fill(int rgb) {fillb=true;fill=rgb;}
  @Override
  void stroke(int rgb) {strokeb=true; stroke=rgb;}
  @Override
  void background(int rgb) {pg.background(rgb>>16,(rgb>>8)&0xff,rgb&0xff);}
  @Override
  void noFill() {fillb=false;}
  @Override
  void noStroke() {strokeb=false;}
  @Override
